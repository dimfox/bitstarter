...
var checkHtmlFile = function(htmlfile, checksfile) {
    ...
    return checkCheerioHtml(cheerioHtml, checksfile);
};
...
var checkHtmlUrl = function(url, checksfile) {
  ...
  restler.get(url).on('complete', function(result) {
    if (result instanceof Error) {
      ...
      var outJson = checkCheerioHtml(cheerioHtml, checksfile);
      console.log(outJson);
    }
  });
};

var checkCheerioHtml = function(cheerioHtml, checksfile) {
...
if(require.main == module) {
    program
        ...
        .option('-u, --url <url>', 'URL to fetch html and then check', clone(assertUrlExists))
        .parse(process.argv);
